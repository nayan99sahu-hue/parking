...
const NAV = [
  ...
  { to: '/admin/parchi-types', label: 'Parchi Types', icon: (
  ...
  { to: '/admin/users', label: 'Operators', icon: (
...
export default function AdminLayout() {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-extrabold text-white text-lg leading-none">ParkiPro</div>
            <div className="text-teal-200 text-xs mt-0.5">Admin Panel</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        <p className="text-teal-300 text-xs font-bold uppercase tracking-widest px-4 mb-2">Main Menu</p>
        {NAV.map(item => (
      ...
      <div className="px-3 py-4 border-t border-white/10 space-y-2">
        ...
        <div className="flex items-center gap-3 px-4 py-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-teal-200 text-xs truncate">Super Admin</p>
          </div>
          <button onClick={handleLogout} title="Logout" className="text-teal-200 hover:text-white transition-colors">
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${dark ? 'dark bg-slate-900' : 'bg-slate-50'}`}>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-sidebar-gradient flex-shrink-0">
        <SidebarContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 bg-sidebar-gradient flex flex-col">
            <SidebarContent />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Topbar */}
        <header className={`flex items-center justify-between px-6 py-4 border-b ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} shadow-sm flex-shrink-0`}>
          <button className="md:hidden text-slate-600" onClick={() => setMobileOpen(true)}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <div className={`text-sm font-medium ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
            {new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </div>
          <div className="flex items-center gap-3">
            <span className={`text-sm font-semibold ${dark ? 'text-slate-200' : 'text-slate-700'}`}>{user?.name}</span>
